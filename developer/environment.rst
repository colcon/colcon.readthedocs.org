...
For the former case it is desired to provide a script to perform the environment update to the user for ease of use.
For the latter case it is a necessity to automate the process in order to build packages in topological order without user interaction.
E.g. if a package installs an executable which should be invokable by name the containing directory needs to be part of the ``PATH`` environment variable.
...
.. note::
    ...
    An executable is unable to change the environment of the current shell.
...
In the root of the install prefix path two kinds of scripts are being generated by ``colcon``:
...
Commonly duplicate values are not useful and only increase the length and decrease the readability.
Therefore the shell scripts try to avoid adding duplicate values where applicable.
...
Also parsing and interpreting ``.dsv`` files would likely not be much faster then invoking the native scripts.
...
When sourcing / call a workspace-level setup file the number of evaluated scripts and / or interpreted ``.dsv`` files can be significant.
